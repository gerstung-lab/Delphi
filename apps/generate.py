...
class Generator:
    ...
    def __init__(self, cfg: GeneratorConfig, model: Delphi):
        self.config = cfg
        self.model = model
    ...
    def generate_one_batch(
        ...
        for _ in range(self.config.max_new_tokens):
            ...
            if (
                self.config.no_repeat
            ):  # TODO: find out why no_repeat would be set to true
                fill = idx + 0
                fill[fill == 1] = 0
            ...
            if self.config.simulate_comorbid and max_comorbid > 1:
                idx_next = torch.zeros(
                    idx_cond.shape[0], max_comorbid, device=idx.device, dtype=torch.long
                )
                ...
                for n_comorbid in range(1, max_comorbid + 1):
                    subject_idx, comorbid_tokens = torch.nonzero(
                        torch.logical_and(
                            probs > self.config.comorbid_cutoff,
                            (comorbid_count == n_comorbid).unsqueeze(-1),
                        ),
                        as_tuple=True,
                    )
                    ...
                    t_next = torch.clamp(
                        -torch.exp(-comorbid_logits)
                        * torch.rand(comorbid_logits.shape, device=idx.device).log(),
                        min=0,
                        max=DAYS_PER_YEAR * 80.0,
                    )
                    t_next = t_next[:, torch.randperm(n_comorbid)]
                    age_next[subject_idx, :n_comorbid] = (
                        age_cond[subject_idx, -1].unsqueeze(-1) + t_next
                    )
            else:
                t_next = torch.clamp(
                    -torch.exp(-logits)
                    * torch.rand(logits.shape, device=idx.device).log(),
                    min=0,
                    max=DAYS_PER_YEAR * 80.0,
                ).min(1)
            ...
            if torch.all(idx_next == self.model.config.vocab_size - 1) or torch.all(
                age_next > self.config.max_age_in_years * DAYS_PER_YEAR
            ):
                break
        ...
        pad = (
            ...
            > 1
        ) + (age > self.config.max_age_in_years * DAYS_PER_YEAR)
    ...
    def generate(
        ...
        for _ in range(n_repeats):
            with torch.no_grad():
                for dd in tqdm(
                    zip(
                        *map(lambda x: torch.split(x, self.config.batch_size), (d0, d1))
                    ),
                    total=len(d0) // self.config.batch_size + 1,
                ):
                    mm = self.generate_one_batch(
                        dd[0].to(
                            self.config.device
                        ),  # TODO: check if moving to device is redundant here
        ...
        a = [
            np.pad(
                x[0].cpu(), ((0, 0), (0, max_len - x[0].shape[1])), constant_values=0
            )
            for x in oo
        ]
        b = [
...
def load_prompt(
    ...
    val = np.fromfile(gen_cfg.data_memmap, dtype=np.uint32).reshape(-1, 3)

    val_p2i = get_p2i(val)

    if gen_cfg.subsample > 0:
        ix = range(0, gen_cfg.subsample, 1)
    else:
    ...
    start_age_in_days = gen_cfg.start_age_in_years * DAYS_PER_YEAR

    w = np.where(
        (d[1].cpu().detach().numpy() <= start_age_in_days).any(1)
        * (d[3].cpu().detach().numpy() >= start_age_in_days).any(1)
    )
    # select everything
    ...
    d1[d1 > start_age_in_days] = -10000.0

    if gen_cfg.start_age_in_years > 0:
...
def main():
    ...
    gen_cfg = dataclass_from_dict(GeneratorConfig, cfg, strict=False)
    print(gen_cfg)

    dump_dir = os.path.join(cfg.ckpt_path, gen_cfg.name)
    ...
    d0, d1 = load_prompt(gen_cfg=gen_cfg)

    a, b = gen.generate(model=model, d0=d0, d1=d1)
    np.save(arr=a, file=os.path.join(dump_dir, "token.npy"))
    np.save(arr=b, file=os.path.join(dump_dir, "time.npy"))
