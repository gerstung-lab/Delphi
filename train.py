...
class TrainConfig: 
    ...
    dtype: str = 'float32'  # 'bfloat16' # 'float32', 'bfloat16', or 'float16', the latter will auto implement a GradScaler
...
def train(cfg: TrainConfig): 
    ...
    ptdtype = {'float32': torch.float32, 'float64': torch.float64,
            'bfloat16': torch.bfloat16, 'float16': torch.float16}[cfg.dtype]
    ...
    def estimate_loss():
        ...
        for split in ['train', 'val']:
            ...
            for k in range(cfg.eval_iters):
                ...
                X, A, Y, B = get_batch(ix, data, p2i, block_size=cfg.model.block_size,
                                    device=cfg.device, select='left',
    ...
    X, A, Y, B = get_batch(ix, train_data, train_p2i, block_size=cfg.model.block_size, device=cfg.device,
    ...
    while True:
        ...
        for micro_step in range(cfg.gradient_accumulation_steps):
            ...
            X, A, Y, B = get_batch(ix, train_data, train_p2i, block_size=cfg.model.block_size, device=cfg.device,
                                padding='random', lifestyle_augmentations=True, select='left',
                                no_event_token_rate=cfg.no_event_token_rate, cut_batch=True)
