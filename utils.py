...
def get_batch(ix, data, p2i, select='center', index = 'patient', padding='regular', block_size=48, device='cpu'):
    """
    ...
    if select == 'right':
        this_i = block_size
    if select in ['smart_random', 'smart_right']:
        this_i = 0
    ...
    tokens = torch.stack([torch.from_numpy(data[i:i+block_size+1,2].astype(np.int64)) for i in ix])
